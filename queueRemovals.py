...
def findPositions(arr, x):
  ...
  for index, value in enumerate(arr):
    queue.append((value, index+1))
  
  for _ in range(x):
    temp = []
    limit = len(queue) if len(queue) < x else x
    for j in range(limit):
      temp.append(queue.pop(0))

    # -item[1]: This is the negative of the original index of the element. We use the negative index to ensure that if there are multiple elements with the  same maximum value, the one with the smallest index (earliest in the original array) is chosen. By negating the index, we effectively prioritize smaller indices since -1 is greater than -2, etc.
    max_num = max(temp, key = lambda item: (item[0], -item[1]))
    output.append(max_num[1])

    for t in temp:
      if t != max_num:
        decrement_value = t[0] - 1 if t[0] > 0 else 0
        queue.append((decrement_value, t[1]))
